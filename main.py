...
if model_name == 'alexnet':
    if params_exp['params_compile']['optimizer'] == 'sgd':
        lr = params_exp['optimizer']['learning_rate']
        momentum= params_exp['optimizer']['momentum']
        sgd = SGD(learning_rate=lr, momentum=momentum)
        params_exp['optimizer'] = sgd
...
model = build_and_compile_model(model_name,params_exp['initializers'], size, params_exp['params_compile'])()
model.summary()
...
if not params_exp['load_weights']:
    ...
    model.load_weights(params_exp['checkpoint_name'] +'.hdf5')
